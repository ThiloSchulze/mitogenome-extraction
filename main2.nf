...
//    path('s*_mitogenome.fa'), emit: strand_test
    ...
    then
      ...
      np_out='NOVOPlasty_out'
      manage_novoplasty=\$( 
      mv contigs_tmp_Mitogenome.txt log_Mitogenome.txt \$np_out
      if [[ -f "Merged_contigs_Mitogenome.txt" ]]
      then
        mv Merged_contigs_Mitogenome.txt \$np_out
      fi
      ...
      then
        ...
        mv Circularized_assemblies_1_Mitogenome.fasta \$np_out
        break
      ...
      then
          ...
          do
              if [[ \$( grep -v '^>' \$blastn_result | wc -m | awk '{print int(\$1+0.5)}' ) -gt '14000' ]]
              then
          ...
          do
              ...
              then
                  ...
                  then
                    ...
                    echo "\$manage_novoplasty" > error_test.txt
                    \$manage_novoplasty
...
process annotate_mitogenome {
    ...
    then
    bfg -F @nad4l -f individual_genes_nuc/nad4.fna -v > individual_genes_nuc/nad4.fna
    fi

    if [[ -f individual_genes_prot/nad4.fna ]]
    then
    bfg -F @nad4l -f individual_genes_prot/nad4.fna -v > individual_genes_prot/nad4.fna
    fi
